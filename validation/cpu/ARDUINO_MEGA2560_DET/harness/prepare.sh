...
mkdir -p "$HOSTDIR"
cc -std=c11 -O2 -fwrapv -DFP_DET_GRID_EMIT -I"$HERE/include" \
    "$HERE/src/fp_determinism.c" -o "$HOSTDIR/grid_emit"
"$HOSTDIR/grid_emit" > "$HERE/include/fp_det_grid.h"

# Host reference: build the copied sources with the stdio main — in the same
# FP_DET_EXTERNAL_GRID mode the firmware uses, consuming the header generated
# above — and run against the committed golden (fails prepare on mismatch),
# then extract the hash.
cc -std=c11 -O2 -fwrapv -DFP_MATH_WITH_STDIO -DFP_DET_EXTERNAL_GRID -I"$HERE/include" \
